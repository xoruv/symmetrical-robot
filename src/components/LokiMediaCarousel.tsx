...
export default function LokiMediaCarousel({ media }: { media: string[] }) {
    ...
    return (
        <div className="flex flex-col items-center">
            <img 
                src="/header.webp" 
                alt="Loki.lol Header" 
                className="w-auto h-[150px] object-contain mb-[-1rem]"
            />
            <div className="scrollbar flex w-full snap-x snap-mandatory overflow-x-auto rounded-lg media-carousel">
                ...
                {media.map((src, i) => (
                    <div
                        ...
                        key={i}
                        className="flex w-full flex-shrink-0 snap-center justify-center items-center rounded-lg"
                    >
                        {src.includes('.mp4') ?
                            <video
                                src={src}
